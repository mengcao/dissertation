...
\section{Action of Generalized Harmonic Formulation}
...
With the Einstein-Hilbert action(\ref{action}) defined explicitly above, we are now able to construct the Hamiltonian formalism of GH formulation. Hamiltonian and equations of motion for GH system is spelled out in Sec.~\ref{hamiltonian}. An extension of the Hamiltonian for the purpose of general covariance is discussed in Sec.~\ref{extension}. In the end, a hyperbolicity analysis of the system is presented in Sec.~\ref{hyperbolicity}. 
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
...
One may notice that Eqs.~(\ref{non covariant hamilton}) are not explicit covariant under foliation preserving coordinate transformation. This is a direct result from the fact that the action itself i.e., Eq.~(\ref{non covariant action}) is not covariant. It is easy to notice that the presence of non-covariant time derivative of coordinate variables, such as $P^{ab}{\dot g}_{ab}$, $\pi{\dot \alpha}$, $\rho_{a}{\dot \beta}$, $\Omega{\dot H}_{\perp}$ and $\Omega_{a}{\dot H}^{a}$, prevents the action from being general covariant. However, a more careful observation reveals that, non-coincidentally, we can extract some terms from the hamiltonian $\mathscr{H}$ to combine with the regular time derivatives and make them covariant time derivatives as presented in Eqs.~(\ref{covariant lapse and shift}) and Eq.~(\ref{covariant spatial metric}). Therefore, we start to modify the action presentation as following
...
\end{split}
\end{align}
Note that here we absorb $\Lambda$ and $\Lambda^{a}$ into functions ${\hat \Lambda}$ and ${\hat \Lambda}^{a}$, respectively. Now we can choose ${\hat \Lambda}$ and ${\hat \Lambda}^{a}$ appropriately such that they equal to the complement terms for ${\dot H}_{\perp}$ and ${\dot H}^{a}$, i.e., 
\begin{subequations}
\begin{align}
...
\end{equation}
This is the reason why we want to replace $\rho_{a}D_{t}\beta^{a}$ with $\rho_{a}D_{t}\Delta \beta^{a}$, since $\rho_{a}\Delta \beta^{a}$ is a weight 1 density under spatial diffeomorphism and a scalar uner time reparameterization while $\rho_{a}\beta^{a}$ is not. 

The product rule of covariant time derivative leads to the definitions of covariant derivatives of momenta as following
\begin{subequations}
\begin{align}
...
\end{align}
\end{subequations}
With these two properties of covariant time derivative, we can vary the covariant action to obtain the explicitly covariant Hamilton's equations,
...
\end{align}
\end{subequations}
The eigenvalues for the scalar sector are $\pm 1$ and 0, each with multiplicity four. The eigenvalues for the vector sector are $\pm 1$ with multiplicity two and 0 with multiplicity two as well. The eigenvalues for the trace-free tensor sector are $\pm 1$. The eigenvectors for all the three sectors are complete, which yields to the conclusion that the system is strongly hyperbolic. 
...
\end{align}
\normalsize
where the tensors $M^{abef}$, $M^{ab}$ and $M$ being positive definite. Using Eqs.~(\ref{symmetric hamilton}) shows that the time derivative of $\epsilon$ has a principal part that can be written as the gradient of a vector $\phi^{a}$; that is, ${\dot \epsilon} \cong \partial_{a}\phi^{a}$. This shows that $\epsilon$ is a quadratic, positive-definite energy density with flux $\phi^{a}$. According to Gundlach and Martin-Garcia\cite{Gundlach:2005ta} that the system (\ref{non covariant hamilton})  and (\ref{covariant hamilton}) are symmetric hyperbolic. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Summary}\label{summary}
The Hamiltonian formulation of Generalized Harmonic general relativity is presented explicitly in this paper. The Hamilton's equations are derived and its symmetric hyperbolicity is verified. We also present the general covariant form of the Hamiltonian and the Hamilton's equations. Furthermore, the Hamiltonian is extended in the purpose of giving some guidance in constructing the gauge condition for gauge source vectors $H_{\perp}$ and $H^{a}$. The symmetric hyperbolicity of this extended formulation is further verified. 
