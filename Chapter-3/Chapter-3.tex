...
\end{eqnarray}
\end{subequations}
They consist of time reparameterizations (\ref{foliationpreservingtransf}a) and {\em time--dependent} spatial coordinate transformations 
...
are not suited for numerical applications. Current numerical codes use one of several alternative systems, such as the 
...
The gauge conditions in current use are not generally covariant. In particular, the ``moving puncture gauge"\cite{Beyer:2004sv, Bona:1994dr, Alcubierre:2002kk}, commonly used with BSSN, is not covariant. 
...
With the foliation preserving transformations being defined in the previous section, we start to investigate transformation rules for various important $3 + 1$ variables under this coordinate transformation, such as $g_{ab}$, $\alpha$ and $\beta^{a}$. More specifically, we determine which physical variables transform tensorially under foliation preserving transformation. For an arbitrary tensor density $T^{a_{1}...a_{n}}_{~~~~~~~~b_{1}...b_{m}}$, if it transforms as
\begin{equation}
...
\end{equation}
it is regarded as ``transforming tensorially" in this context. $\left|\frac{\partial t}{\partial t'}\right|$ and $\left|\frac{\partial x}{\partial x'}\right|$ are the Jacobian matrix determinant of time reparameterization and spatial diffeomorphisms, respectively. We refer to this class of variables as a weight $i$ density under time reparameterization and a type $n \choose m$ weight $j$ tensor density under time dependent spatial diffeomorphisms. Furthermore, if one quantity turns out to be a non-tensor, it will be ``tensorized" by combining with background metric terms, other physical variables and their derivatives. 

As discussed in Ref.~\cite{Brown:2010rya, Brown:2011qg}, a background metric is introduced to achieve general covariance of non-tensorial physical variables. An analogy of this procedure is the use of background connection in spacetime coordinate transformations. It is well recognized that the four dimensional physical connection $\Gamma^{\alpha}_{~\mu\nu}$ is a non-tensorial variable. However, the difference between the physical connection and a background connection, $\Delta \Gamma^{\alpha}_{~\mu\nu} \equiv \Gamma^{\alpha}_{~\mu\nu} - {\bar \Gamma}^{\alpha}_{~\mu\nu}$, is a type $1 \choose 2$ tensor. The fact that background metric terms are often neglected from practical numerical equations can be justified by choosing the background metric to be flat. 

We start by listing several important identities revealed by the transformation rule for the delta symbol. The inverse transformation of Eqs.~(\ref{foliationpreservingtransf}) is  
\begin{subequations}
\begin{eqnarray}
t & = & t(t')\\
x^{a} & = & x^{a}(t',x^{a'}) \ .
\end{eqnarray}
\end{subequations}
Using the chain rule of partial differentiation to inspect the delta function for this transformation, we have
...
\end{equation}
Since $t'\left(t\right)$ is independent of $x^{a}$, the term with $\partial t'/\partial x^{b}$ vanishes in Eq.~(\ref{chainrule}). Hence we obtain an important identity for foliation preserving coordinate transformations
...
\frac{\partial x^{a'}}{\partial x^{a}}\frac{\partial x^{a}}{\partial x^{b'}} = \delta^{a'}_{b'} \ .
\end{equation}
Although they are not directly referred to, Eqs.~(\ref{delta}, \ref{deltainverse}) are extensively used in the following derivations. We list them here for readers who want to reproduce our work. 

To derive the transformation rules of the spatial metric $g_{ab}$, lapse function $\alpha$ and shift vector $\beta^{a}$, we need to start with the spacetime metric $^{(4)}g_{\mu\nu}$. The spacetime metric transforms tensorially under this transformation, i.e., $^{(4)}g_{\mu' \nu'} =~^{(4)}g_{\mu\nu}\frac{\partial x^{\mu}}{\partial x^{\mu'}}\frac{\partial x^{\nu}}{\partial x^{\nu'}}$. Plugging it into Eq.~(\ref{spacetime metric 3 + 1}), we have
\begin{equation}\label{spacetime metric}
^{\left(4\right)}g_{\mu'\nu'} = \left(g_{ab}X^{a}_{\mu}X^{b}_{\nu} - n_{\mu}n_{\nu}\right)\frac{\partial x^{\mu}}{\partial x^{\mu'}}\frac{\partial x^{\nu}}{\partial x^{\nu'}} \ .
\end{equation}
By substituting indices $\mu'$ and $\nu'$ for $a'$ and $b'$ respectively in Eq.~(\ref{spacetime metric}), we can obtain the transformation rule of spatial metric as follows
...
With the transformation rules of the spatial metric and its related quantities listed in Eqs.~(\ref{spatialmetric} - \ref{spatialmetricdet}), we move on to investigate the transformation rules for the lapse function and shift vector. 
...
\end{align}
Using the results from Eq.~(\ref{spatialmetricinverse}) and Eq.~(\ref{shiftcovectortransform}), we can easily derive the transformation rule of the shift vector
...
\end{align}
From the calculation above we can tell that neither the shift vector $\beta^{a}$ nor its covector $\beta_{a}$ transforms tensorially under this change of coordinates. This issue can be addressed by subtracting the background shift vector from the physical shift vector. The background shift vector transforms similarly to the physical shift vector, i.e., 
...
\end{equation}
This combination serves as the covariant version of $\beta^{a}$. 

Using the result from Eq.~(\ref{shiftvectortransform}) along with Eq.~(\ref{lapse}) and Eq.~(\ref{spacetime metric}), we can derive the transformation rule of the lapse function, 
\begin{align*}
...
The transformation rules for the important $3 + 1$ terms with zero-order time and spatial derivatives are written above. However, since the Einstein field equations are a system of partial differential equations, we still seek to understand how partial derivative terms transform under the foliation preserving transformation. 

In order to study how partial derivatives transform, let's review the chain rule for this transformation briefly. The chain rule in the spacetime domain states
\begin{equation}
\partial_{\mu'} = \frac{\partial x^{\mu}}{\partial x^{\mu'}}\partial_{\mu} \ .
\end{equation}
Therefore, if $\mu' = t'$, we have
...
According to this analysis, the spatial Christoffel symbols transform as its spacetime counterpart under spacetime coordinate transformations, due to the fact that it is only constructed from the spatial metric and its spatial derivatives, 
\begin{equation}
...
\end{equation}
The covariant spatial Christoffel symbols can be easily obtained by subtracting the background connection, $\Delta \Gamma^{a}_{~bc} \equiv \Gamma^{a}_{~bc} - {\bar \Gamma}^{a}_{~bc}$. It is easy to confirm that $\Delta\Gamma^{a}_{~bc}$ transforms as a type $1 \choose 2$ tensor under spatial diffeomorphism and a scalar under time reparameterization, i.e.,  
\begin{equation}
...
The transformation of the shift covector's spatial covariant derivative can be derived as follows, 
...
\end{align} 
It is obvious that general covariance of this expression can be achieved by replacing $\beta_{a}$ by its covariant counterpart $\Delta \beta_{a}$. This gives a type $0 \choose 2$ tensor under spatial diffeomorphism and a weight 1 scalar density under time reparameterization, 
...
\end{equation}	
The spatial covariant derivative of $\alpha$, which takes the form of a regular spatial partial derivative, transforms as 
...
\end{align}
Thus, $\partial_{a}\alpha$ is a type $0 \choose 1$ tensor under spatial diffeomorphism and a weight 1 density under time reparameterization.

Due to the extra terms introduced by Eq.~(\ref{timederivative}), transformations of time derivatives pose a more interesting case. Let's first examine the time derivative of the spatial metric. It transforms as, 
\begin{align}
...
\end{align}
Hereby we introduce the notion of {\em covariant time derivative} under foliation preserving transformations, $D_{t}$, such that $D_{t}T^{A}$ transforms tensorially, where $T^{A}$ is an arbitrary tensor density whose indices are denoted $A$. For instance, we require the covariant time derivative of the spatial metric to transform as
\begin{equation}
	D_{t'}g_{a'b'} = D_{t}g_{ab}\frac{\partial x^{a}}{\partial x^{a'}}\frac{\partial x^{b}}{\partial x^{b'}}\frac{\partial t}{\partial t'} \ .
\end{equation}
By revisiting the definition of the extrinsic curvature in Eq.~(\ref{extrinsic}), a keen observer will notice that $D_{t}g_{ab}$ should be closely related to the extrinsic curvature $K_{ab}$. Since $K_{ab}$ is a pure spatial tensor, it transforms tensorially as
\begin{equation}
	K_{a'b'} = K_{ab}\frac{\partial x^{a}}{\partial x^{a'}}\frac{\partial x^{b}}{\partial x^{b'}} \ .
\end{equation}
Hence we can define the covariant time derivative of the spatial metric, $D_{t}g_{ab}$, as
...
\end{equation}
Furthermore, it is straightforward to verify that for any field that transforms as a scalar under time reparameterization, the covariant time derivative operator can be defined as
\begin{equation}
	D_{t} \equiv \partial_{t} - \mathcal{L}_{\beta} \ .
\end{equation}
Covariant time derivatives for the lapse function and shift vector are more complicated and we are required to study the covariant time derivative of these two variables more carefully. 

We start by writing down the transformation rule for the regular time derivative of the lapse function, as follows
...
\end{equation}
One can check that its transformation rule turns out to be exactly as expected, 
\begin{equation}
	D_{t'}\alpha' = D_{t}\alpha \left(\frac{\partial t}{\partial t'}\right)^{2} \ .
\end{equation}

Since $\Delta \beta^{a}$ is the tensorial version of $\beta^{a}$, the covariant time derivative of the shift vector should be constructed from $\partial_{t}\Delta \beta^{a}$. The transformation rule for $\partial_{t}\Delta \beta^{a}$ can be written as
...
\end{align}
Based on the transformation rules of the shift vector and its covariant spatial derivative, as listed in Eqs.~(\ref{shiftvectortransform}, \ref{shiftvectorcovariantspatialderivativetransform}, \ref{covariantshiftvectorcovariantspatialderivativetransform}), we propose the following ansatz for $D_{t}\beta^{a}$
\begin{equation}
...
The process of determining values of these four constants is tedious so we skip directly to the conclusion here. In order to achieve the covariant time derivative of the shift vector, the constants $c_{1}$, $c_{2}$, $c_{3}$ and $c_{4}$ must satisfy
\begin{align*}
c_{1} + c_{3} & = -1\\
c_{2} + c_{4} & = 1\\
c_{1} + c_{2} & = 1\\
...
\end{equation}
In both definitions of $D_{t}g_{ab}$ and $D_{t}\alpha$, the covariant time derivative operators share the same pattern as $\partial_{t} - \beta^{b}\partial_{b}$. In order to include the same pattern in $D_{t}\beta^{a}$, an appropriate choice for $\sigma$ is $\sigma = -1$. Therefore, in the rest of this dissertation, the covariant time derivative of the shift vector is defined as
...
The transformation rule for the gauge source term of the generalized harmonic formalism needs to be examined as well, but first we need to define its 3 + 1 decomposition. The decomposition splits the spacetime gauge source term into a spatial component
...
& = H_{\perp} \ .
\end{align}
Hence we conclude that $H_{\perp}$ acts as a scalar under both spatial diffeomorphism and time reparameterization. 

As mentioned before, for any field that transforms as a scalar under time reparameterization, its covariant time derivative operator can be expressed as $D_{t} \equiv \partial_{t} - \mathcal{L}_{\beta}$. Therefore, for the covariant time derivative of $H_{\perp}$ and $H_{a}$, we have that
\begin{subequations}
	\begin{align}
	...
	\end{align}
\end{subequations}
This concludes our study of transformation rules under foliation preserving coordinate transformations. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Covariant Gauge Condition}\label{gauge}
With the transformation rules for all important field variables under foliation preserving transformation listed in Sec.~\ref{transform}, we are now able to revisit the non-covariant gauge conditions mentioned in Sec.~\ref{generalcovariancefoliationpreservingtransformation}. In this section, we recover general covariance of various gauge conditions used in different numerical relativity formulations. The strategy for making gauge conditions covariant includes two steps. First, replace non-tensorial terms with their tensorial counterparts. For instance, replace $\partial_{t}\alpha$ with $D_{t}\alpha$ or substitute $\partial_{t}\beta^{a}$ with $D_{t}\beta^{a}$. The second step is balancing tensor density weights on both sides of the equation. Multiplying by ${\bar \alpha}$ increases weight density by 1 under time reparameterization while multiplying by $\sqrt{{\bar g}}$ increase weight density by 1 under spatial diffeomorphism. This strategy is illustrated in Sec.~\ref{movingpuncture} and Sec.~\ref{dampedwave}. Sec.~\ref{movingpuncture} discusses the gauge condition used in BSSN formalism and Sec.~\ref{dampedwave} presents a general covariant representation of the damped-wave gauge condition\cite{Pretorius:2006tp, Szilagyi:2009qz, Lindblom:2009tu} employed by the generalized harmonic community. 

\subsection{Moving puncture gauge condition}\label{movingpuncture}
Moving puncture gauge condition consists of two parts. The 1 + log slicing condition governs evolution of the lapse function and the Gamma-driver shift condition controls the shift vector. 

The 1 + log slicing condition reads as
\begin{equation}\label{1 + log slicing}
\partial_{t}\alpha - \beta^{c}\partial_{c}\alpha = -2\alpha K \ .
\end{equation}
Notice that the left hand side shares several terms with $D_{t}\alpha$. It is intuitive and appropriate to replace the left hand side with $D_{t}\alpha$. However, this substitution makes the left hand side transform as a weight 2 density under time reparameterization while the right hand side transforms as a weight 1 density under such transformations. Therefore, we multiply by the background lapse function ${\bar \alpha}$, which transforms as a weight 1 density, on the right hand side to balance the density weights. This leads to the generally covariant 1 + log slicing condition, as follows
\begin{equation}\label{covariant 1 + log slicing}
D_{t}\alpha = -2\alpha{\bar \alpha}K \ .
\end{equation}
Note that this equation reduces to the original Eq.~(\ref{1 + log slicing}) with the choice of a trivial background metric. 
...
\end{align}
\end{subequations}
where ${\tilde \Gamma}^{a}$ is the trace of the conformal Christoffel symbols can be expressed as\footnote{The formal definition of ${\tilde \Gamma}^{a}$ is in terms of the conformal metric and the conformal Christoffel symbols: ${\tilde \Gamma}^{a} \equiv {\tilde g}^{bc}{\tilde \Gamma}^{a}_{~bc}$. }
\begin{equation}
...
In this way we can recover general covariance while restricting $B^{a}$ to transform as a type $1 \choose 0$ tensor under spatial diffeomorphism and a scalar under time reparameterization. Manipulating Eq.~(\ref{gammadrivershiftfirst}) in this way yields, 
...
D_{t}\Delta{\tilde \Gamma}^{a} & \equiv \left(\partial_{t} - \mathcal{L}_{\beta}\right)\Delta{\tilde \Gamma}^{a} \ .
\end{align}
\end{subequations}
In summary, Eqs.~(\ref{gammadrivershiftfirstcovariant}, \ref{gammadrivershiftsecondcovariant}) represent generally covariant Gamma-driver shift condition as a system of two equations. The covariant equations also reduce to the original Gamma-driver shift equations (\ref{gammadrivershiftset}) when a trivial background metric is chosen. 

In some studies\cite{vanMeter:2006vi}, Eqs.~(\ref{gammadrivershiftset}) are integrated with initial conditions $\beta^{a}\left(t=0\right) = B^{a}\left(t=0\right) = {\tilde \Gamma}^{a}\left(t = 0\right) = 0$ to yield a single-equation form of Gamma-driver shift condition
...
\end{equation}
Due to its simplicity, we focus our discussion in the rest of this dissertation on the covariant Gamma-driver shift gauge of Eq.~(\ref{covariant Gamma-driver shift}) instead of Eqs.~(\ref{gammadrivershiftfirstcovariant}, \ref{gammadrivershiftsecondcovariant}).
...
The damped-wave gauge condition for $H^{a}$ is
\begin{equation}\label{damped gauge vector}
H^{a} = -\mu_{S}\frac{\beta^{a}}{\alpha}
\end{equation}
where $\mu_{S}$ is a numerical damping factor.

For the other gauge source term $H_{\perp}$, the damped-wave gauge condition is
...
The only non-tensorial term in Eq.~(\ref{damped gauge vector}) is the shift vector $\beta^{a}$ and we can easily fix it by replacing $\beta^{a}$ with $\Delta \beta^{a}$. Since both $\Delta \beta^{a}$ and $\alpha$ act as weight 1 tensor densities under time reparameterization, $\Delta \beta^{a}/\alpha$ simply transforms as a scalar under time reparameterization, which is the same case for $H^{a}$. Therefore, we can write the covariant form of Eq.~(\ref{damped gauge vector}) as
...
\end{equation} 
so that it is manifestly covariant. Under the choice of a trivial background metric, Eqs.~(\ref{covariant damped gauge vector}, \ref{covariant damped gauge scalar}) also reduce to the original damped-wave gauge conditions (\ref{damped gauge vector}, \ref{damped gauge scalar}), respectively. 

As mentioned in the beginning of this chapter, generally covariant gauge conditions enjoy superior preference in numerical simulations when change of coordinates is necessary, since the equations are invariant under coordinate transformations. Another way to obtain the correct gauge conditions under a coordinate transformation is to transform the non-covariant gauge condition explicitly. We briefly discuss the equivalence of these two methods in Sec.~\ref{transformgauge} and in Sec.~\ref{example} we demonstrate these two methods in action under a specific kind of coordinate transformation. It is further proved that these two methods yield equivalent results. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Another Way to Transform the Gauge Condition}\label{transformgauge}
Assume we have some variables $v$. Under a change of coordinates, they transform as $v' = T\left(v\right)$ where $T$ is some function. We also introduce a 
...
conditions in the original coordinates are $F^A\left(v,\bar c\right) = 0$. (This is the non--covariant version of the gauge conditions.) As we discussed above, we have two ways to derive the transformed gauge conditions. One way: 
\begin{itemize}
...
\end{equation}
This is guaranteed by the Eq.~(\ref{covariant equivalence}) above. In Eq.~(\ref{covariant equivalence}), just let $v = T^{-1}\left(v'\right)$ and $\bar v = \bar c$, which implies $\bar v' = T\left(\bar c\right)$. Later in this dissertation, we refer the first method as the {\em covariant method} and the second method as the {\em transformed method}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Time Dependent Spatial Rotation Coordinate Transformation}\label{example}
A dual coordinate frame system\cite{Scheel:2006gg} is extensively employed in various binary black hole coalescence simulations\cite{Scheel:2008rj, Szilagyi:2009qz} by the CalTech/Cornell numerical relativity collaboration. This dual-frame technique is one of the motivations for our study of the study general covariant gauge conditions. In the dual-frame system, a comoving frame is rotating with respect to an asymptotically Minkowski inertial frame and the numerical simulation is build upon these two coordinate systems. In this section, a simple time dependent spatial rotation coordinate transformation is defined to demonstrate how to obtain correct gauge conditions under this change of coordinates. 

In this coordinate transformation, we keep the $t$ coordinate intact while letting the spatial coordinates rotate around the $z$ axis at a constant angular velocity $\omega$. Hence the transformed coordinates can be expressed in terms of the original coordinates as below
\begin{subequations}
\begin{align}
t' & = t\notag\\
...
\end{equation}
One would argue that this equation is identical to the regular 1 + log slicing equation. This is only true under this specific coordinate transformation, especially when we have $\partial t/\partial t' = 1$. 

According to Eq.~(\ref{backgroundshiftvector}) and with the choice ${\bar \beta}^{a} = 0$, we have
\[
...
\end{align}
\end{subequations}
Eqs.~(\ref{transformed1+logslicing}, \ref{transformedgammadrivershift}) are the gauge conditions obtained by the covariant method. To verify their correctness, we can derive identical gauge conditions by carrying out the transformed method in the rest of this section. 

According to the transformation rules we obtained in Sec.~\ref{transform}, we write the unprimed variables in terms of the primed variables as follows
...
\end{align}
\end{subequations}
Plugging Eqs.~(\ref{unprimedtoprimed}) into Eq.~(\ref{1 + log slicing}), we show that it can be reduced to the same equation as Eq.~(\ref{transformed1+logslicing}), the transformed 1 + log slicing gauge from covariant method. 
\begin{align}
...
\partial_{t'}\alpha'  - \beta^{c'}\partial_{c'}\alpha' & = -2\alpha' K' \ .
\end{align}
To obtain Gamma-driver shift gauge under this change of coordinates via the transformed method, we use Eqs.~(\ref{unprimedtoprimed}) to rewrite Eq.~(\ref{gammadrivershift}) in terms of the primed variables. Since this is a long equation, let's first examine the left hand side, 
\begin{align}
...
\end{equation}
One can easily check that, for each component, Eq.~(\ref{invertedgammadrivershift}) is equivalent to Eqs.~(\ref{transformedgammadrivershift}).

Hereby it is proved that the covariant method and the transformed method lead to identical moving puncture gauge conditions under a change of coordinates. This shows that our derivation of covariant gauge conditions is valid. 

The purpose of this chapter is to lay down the foundation for developing a covariant Hamiltonian formulation of the GH equations. Achieving general covariance is one of the most important aspects for deriving the action and Hamiltonian for GH system. As illustrated later in this dissertation, with the knowledge of the covariant time derivative, we are able to extend the Hamiltonian and rewrite the non-covariant Hamilton's equations in a generally covariant format.  
